const BLOG = {
  ...
  NEXT_REVALIDATE_SECOND: process.env.NEXT_PUBLIC_REVALIDATE_SECOND || 5, // Update content cache interval, in seconds; that is, each page has a 5-second pure static period, during which no matter how many times it is accessed, it will not fetch Notion data; increasing this value helps save Vercel resources and improve access speed, but it will also cause delays in article updates.
  ...
  AUTHOR: process.env.NEXT_PUBLIC_AUTHOR || 'anonymous', 
  BIO: process.env.NEXT_PUBLIC_BIO || 'nothing', 
  LINK: process.env.NEXT_PUBLIC_LINK || '', 
  KEYWORDS: process.env.NEXT_PUBLIC_KEYWORD || 'Notion', 
  ...
  RANDOM_IMAGE_REPLACE_TEXT:
    ...
    'images.unsplash.com', // URL keywords that trigger image replacement (multiple supported, separated by English commas), only image URLs containing this keyword will be replaced with the random image URL above
    // e.g., images.unsplash.com (all images hosted on Notion will be replaced). If you have already added a random image URL in Notion, and that service happens to go down or shuts down, you can switch all illustrations at once by configuring that URL here
    // By default, it will also replace the homepage cover image and avatar you uploaded to Notion. It is recommended to host the homepage cover image and avatar on another image hosting service and configure the link in Notion.
