...
export async function getConfigMapFromConfigPage(allPages) {
  ...
  if (pageIds?.length === 0) {
    console.error(
      '[Notion Configuration] The retrieved article list is empty, please check the Notion template',
      collectionQuery,
      collection,
      collectionView,
      viewIds,
  ...
  for (let i = 0; i < pageIds.length; i++) {
    ...
    if (properties) {
      ...
      const config = {
        ...
        key: properties['name'] || properties.Name,
...
export function parseConfig(configString) {
  ...
  try {
    ...
    console.error('Error occurred when parsing the eval(INLINE_CONFIG) configuration: ', evalError)
    return {}
  }
}
