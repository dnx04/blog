...
export async function getPageWithRetry(id, from, retryAttempts = 3) {
  if (retryAttempts && retryAttempts > 0) {
    console.log(
      ...
      `from:${from}`,
      `id:${id}`,
      retryAttempts < 3 ? `Remaining retry attempts:${retryAttempts}` : ''
    )
    try {
      ...
      console.log('[API Response]', `Time elapsed:${end - start}ms - from:${from}`)
      return pageData
    } catch (e) {
...
export const fetchInBatches = async (ids, batchSize = 100) => {
  ...
  for (let i = 0; i < ids.length; i += batchSize) {
    ...
    const end = new Date().getTime()
    console.log(
      `[API Response] Time elapsed:${end - start}ms Fetching missing blocks count:${ids.length} `
    )
